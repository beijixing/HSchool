...
- (void)setState:(MJRefreshState)state
{
    ...
    if (state == MJRefreshStateNoMoreData || state == MJRefreshStateIdle) {
        ...
        NSInteger CurrentCount1 = [self totalDataCountInScrollView];
        // 刚刷新完毕
        if (MJRefreshStateRefreshing == oldState && deltaH > 0 && CurrentCount1 != self.lastRefreshCount) {
...
{
    NSInteger totalCount1 = 0;
    if ([self.scrollView isKindOfClass:[UITableView class]]) {
        UITableView *tableView = (UITableView *)self.scrollView;
        
        for (NSInteger section = 0; section<tableView.numberOfSections; section++) {
            totalCount1 += [tableView numberOfRowsInSection:section];
        }
    } else if ([self.scrollView isKindOfClass:[UICollectionView class]]) {
        UICollectionView *collectionView = (UICollectionView *)self.scrollView;
        
        for (NSInteger section = 0; section<collectionView.numberOfSections; section++) {
            totalCount1 += [collectionView numberOfItemsInSection:section];
        }
    }
    return totalCount1;
}
